...
class Dj_App_String_Util_Test extends TestCase {
    ...
    /**
     * A BARE key fills both supported tag forms, so a caller never has to wrap
     * its own data before handing it over.
     */
    public function testReplaceMergeTagsWithBareKeys()
    {
        $template = '%title% | {site_title}';
        $replace_map = [
            'title' => 'Hosting',
        ...
        $this->assertEquals('Hosting | djebel.com', $result);
    }

    /**
     * The same bare key fills a brace tag on its own, so a template may use
     * either delimiter throughout.
     */
    public function testReplaceMergeTagsWithBraceTemplate()
    {
        ...
        $this->assertEquals('Hi Slavi, you have 3 messages', $result);
    }

    /**
     * A template may mix the two forms, and each tag is filled from the same
     * bare key.
     */
    public function testReplaceMergeTagsWithMixedDelimiters()
    {
        ...
        $this->assertEquals('Hi, Slavi!', $result);
    }

    /**
     * A numeric key is a plain int once PHP stores it, so it must still resolve
     * to a tag rather than tripping over the delimiter check.
     */
    public function testReplaceMergeTagsWithNumericKey()
    {
        $template = 'Item %5% and {7}';
        $replace_map = [
            5 => 'five',
            7 => 'seven',
        ];
        ...
        $this->assertEquals('Item five and seven', $result);
    }

    /**
     * A bare key whose tag is absent leaves the template untouched — it must not
     * match the word on its own, only the delimited tag.
     */
    public function testReplaceMergeTagsBareKeyDoesNotMatchPlainWord()
    {
        $template = 'The title stays put';
        $replace_map = [
            'title' => 'REPLACED',
        ];
        ...
        $this->assertEquals('The title stays put', $result);
    }
